...
- **高风险阶段**：对 `dbe`、`stretch`、`star_remove`、`star_process`、`star_combine`、`star_reduce`、`enhance`、`style` 等关键步骤必须逐步审查。
- **低风险步骤**：可合并执行，但最终需进行真实性审查。
- **星点分离优先**：除球状/疏散星团、M45 等“星点即主体”的目标外，大多数深空目标默认应尝试 `star_remove → star_process → star_combine`。轻度 `star_reduce` 只能作为分离失败、质量边际或用户明确要求保守处理时的降级路径，不能静默替代星点分离。
- **星点处理依赖**：管线会自动处理星点相关的前置与后置链路依赖，并在密集星场和本地 StarNet2 可用时自动优先使用 StarNet2。
...
2. **Round 2: 针对问题增强**：根据诊断报告对 DBE、降噪、拉伸、星点分离/重组、细节增强及定调风格进行微调。
...
这份 evidence，避免重复解析 Header。默认禁止网络解析目标名或在线星表。
...
**关键步骤**：analyze → adaptive pipeline (dbe + color + star_remove +
stretch + star_process + final_color + star_combine + local_enhance)
